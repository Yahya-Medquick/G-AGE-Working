...
export const SpecificationsAccordion: React.FC<SpecificationsAccordionProps> = ({
  ...
  useEffect(() => {
    const saved = loadSavedSpecs();
    if (saved) {
      onChangeSpecs({ ...DEFAULT_SPECS, ...saved });
    }
  }, []);

  const concept = specs.concept || DEFAULT_SPECS.concept;
  const exam = specs.exam || DEFAULT_SPECS.exam;
  const research = specs.research || DEFAULT_SPECS.research;

  const handleConceptChange = (key: keyof ConceptSpecs, val: any) => {
    const next = { ...specs, concept: { ...concept, [key]: val } };
    onChangeSpecs(next);
    saveSpecs(next);
  };

  const handleExamChange = (key: keyof ExamSpecs, val: any) => {
    const next = { ...specs, exam: { ...exam, [key]: val } };
    onChangeSpecs(next);
    saveSpecs(next);
  };

  const handleResearchChange = (key: keyof ResearchSpecs, val: any) => {
    const next = { ...specs, research: { ...research, [key]: val } };
    onChangeSpecs(next);
    saveSpecs(next);
  };

  const handleReset = () => {
    onChangeSpecs(DEFAULT_SPECS);
              ...
              <div className="flex rounded-lg bg-slate-200/70 dark:bg-slate-800 p-0.5 border border-slate-200 dark:border-slate-700">
                {(['beginner', 'intermediate', 'advanced', 'expert'] as const).map((lvl) => (
                  <button
                    ...
                    className={`flex-1 py-1.5 text-[11px] font-medium rounded-md capitalize transition-colors cursor-pointer ${
                      concept.level === lvl
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. Federal Board, Cambridge, NUST..."
                  className="w-full px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 text-xs focus:ring-1 focus:ring-amber-500 focus:outline-none"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. Physics, Chemistry, Maths, Biology..."
                  className="w-full px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 text-xs focus:ring-1 focus:ring-amber-500 focus:outline-none"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="e.g. 9th, 10th, FSc, BS / Undergrad..."
                  className="w-full px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 text-xs focus:ring-1 focus:ring-amber-500 focus:outline-none"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              ...
              <div className="space-y-1">
                ...
                <button
                  ...
                  className={`w-full py-1 px-2 rounded-lg border text-xs font-semibold flex items-center justify-between transition-colors cursor-pointer ${
